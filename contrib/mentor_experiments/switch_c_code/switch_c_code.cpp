...
extern "C" float
_ZN13switch_plugin6MyTime9time_loopEv()
{
  printf("in wrapper\n");
  return 0.0;
}
...
restart()
{
  ...
  if (addr != NULL) {
    JNOTE("Found symbol")(getenv("REPLACE_SYMBOL_WITH_DEBUG"))(addr);
    ...
    JNOTE("Did not find symbol")(getenv("REPLACE_SYMBOL_WITH_DEBUG"));
    dlclose(handle);
  }
}
